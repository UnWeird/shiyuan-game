...
exports.generateHexMap = generateHexMap;
exports.isInMapRange = isInMapRange;
exports.isInStartZone = isInStartZone;
exports.isInBattleZone = isInBattleZone;
exports.hexLineDraw = hexLineDraw;
exports.hexRound = hexRound;
exports.hexRange = hexRange;
exports.isInDirection = isInDirection;
exports.getShootingPath = getShootingPath;
exports.getBallistaVerticalPath = getBallistaVerticalPath;
exports.getFanShapedHexes = getFanShapedHexes;
...
function hexNeighbors(hex) {
    return HEX_DIRECTIONS.map(dir => createHex(hex.q + dir.q, hex.r + dir.r));
}
/**
 * 计算两个六边形之间的距离
 */
 ...
 * 判断六边形是否在地图范围内
 */
function isInMapRange(hex, radius) {
 ...
 */
function isInStartZone(hex, playerSide) {
    if (playerSide === 'top') {
 ...
 * @returns 垂直贯穿路径
 */
function getBallistaVerticalPath(from, isPlayerOne, mapRadius) {
    const path = [];
    // 正前方推进的增量
    // 玩家1: (q+1, r-2, s+1)
    // 玩家2: (q-1, r+2, s-1)
 ...
 */
function getFanShapedHexes(from, direction, range, mapRadius) {
    const result = [];
    // 120度扇形：中心方向、左侧60度（逆时针一个方向）、右侧60度（顺时针一个方向）
    const centerDir = direction;
    const leftDir = ((direction + 5) % 6); // 逆时针一个方向（60度）
    const rightDir = ((direction + 1) % 6); // 顺时针一个方向（60度）
    // 获取相邻的3个格子
 ...
 * 生成六边形的SVG路径
 */
function hexToSVGPath(hex, size) {
